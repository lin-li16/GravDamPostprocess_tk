...
def out_acc_data(odb, step, nodelist, fpath):
    ...
    session.odbData[odbname].setValues(activeFrames=((step, ('0:-1', )), ))
    session.xyDataListFromField(odb=odb, outputPosition=NODAL, variable=(('A', 
        NODAL, ((COMPONENT, 'A1'), (COMPONENT, 'A2'), )), ), nodeLabels=(('PART-1-1', (
...
def output_stre_data_last_frame(odb, group, group_type, step, fpath):
    ...
    session.writeFieldReport(fileName=fpath + '.csv', append=OFF, 
        ...
        'Max. In-Plane Principal'), (INVARIANT, 'Min. In-Plane Principal'), )), ))
...
def output_stre_data_env(odb, group, group_type, step, fpath):
    ...
    (maxField_S, maxIndex_S) = visualization.maxEnvelope(all_frames, MAX_INPLANE_PRINCIPAL)
    (minField_S, minIndex_S) = visualization.minEnvelope(all_frames, MIN_INPLANE_PRINCIPAL)
    ...
    description='Stress components (maximum envelope) using Max. In-Plane Principal',
        field=maxField_S)
    ...
    description='Stress components (minimum envelope) using Min. In-Plane Principal',
        field=minField_S)
    ...
    session.writeFieldReport(fileName=fpath + '-min.csv', append=OFF, sortItem='结点编号', 
        ...
        INTEGRATION_POINT, ((INVARIANT, 'Min. In-Plane Principal'), )), ))
...
def output_slide_data_last_frame(odb, group, group_type, step, fpath):
    ...
    session.writeFieldReport(fileName=fpath + '.csv', append=OFF, 
        sortItem='结点编号', odb=odb, step=step_id, frame=num_frames, outputPosition=NODAL, 
        variable=(('S', INTEGRATION_POINT, ((COMPONENT, 'S22'), (COMPONENT, 
        'S12'), )), ))
