...
class Dynamic3D(tk.Frame):
    def __init__(self, parent, root):
        ...
        self.fail_time = tk.StringVar()  # 初始化抗滑稳定安全系数K
        self.figs = []  # 初始化图片列表
    ...
    def read_mesh_geo(self):
        ...
        if not self.mesh_path.get().endswith('.inp'):  # 判断文件格式是否正确
            ...
            self.Nset, self.Elset, self.Surface = read_mesh(self.mesh_path.get())  # 读取所有分组
    ...
    def clear_file_entry(self):
        ...
        self.odb_path.set('')
        self.geo_path.set('')
        self.Nset, self.Elset, self.Surface = [], [], []  # 初始化所有集合
        self.coord_all = {}  # 初始化所有局部坐标系
        self.bounds = []  # 初始化边界列表
        self.areas = []  # 初始化所有结点面积列表
        self.holes = []  # 初始化孔洞列表
        self.concens = []  # 初始化应力集中点列表
        self.csv_flist = []  # 初始化所有结果文件
    ...
    def renew_combobox(self):
        ...
        for f in self.csv_flist:  # 根据odb文件夹中存在的位移数据来获取剖面
            if 'dyn-disp-' in f:
                disp_sec.append(f.replace('dyn-disp-', '').replace('.csv', ''))
        if len(disp_sec) > 1:
            disp_sec.append('全选')
        ...
        for f in self.csv_flist:  # 根据odb文件夹中存在的应力数据来获取剖面
            if 'dyn-stre-env-' in f:
                stre_sec.append(f.replace('dyn-stre-env-', '').replace('.csv', '').replace('-max', '').replace('-min', ''))
        if len(stre_sec) > 1:
            stre_sec = list(np.unique(np.array(stre_sec)))
            stre_sec.append('全选')
        ...
        for f in self.csv_flist:
            if 'dyn-slid-' in f:
                slide_plane.append(f.replace('dyn-slid-', '').replace('.csv', ''))
            ...
            if self.nodes is None:  # 检查是否读取了结点信息
                ...
                if self.geo_path is None:  # 检查是否读取了边界信息
                    ...
                    if os.path.exists(acc_data_path):  # 检查是否存在加速度数据文件
                        ...
                        PGA = self.acc_PGA_entry.get()  # 获取输入地震动的PGA
                            ...
                            for node in acc_node:
                                for direc in acc_direc:
                                    if node != '全选' and  direc != '全选':
                                        ...
                                        if PGA != '':  # 如果给了输入地震动的PGA，则再绘制放大倍数时程曲线
                    ...
                    if disp_sec != '' and disp_direc != '':
                        ...
                        for sec in disp_sec:
                            if sec != '全选':
                                fname = os.path.join(odb_dir, 'dyn-disp-%s.csv' % sec)
                                ref_node = self.disp_ref_entry.get()  # 获取参考点
                                ...
                                disp_max, disp_min, xxU, yyU = dyn_disp(fname, coordinate_nodes, self.nodes, ref_node)  # 处理动力位移数据
                                # 获取剖面的边界文件
                                bounds = []
                                    ...
                                    if direc != '全选':
                                        if len(bounds) > 0:
                                            ...
                                            for ss in sign:
                                                if direc == 'X' and ss == '正':
                                                    zzU = disp_max[:, 0] * 100
                                                elif direc == 'X' and ss == '负':
                                                    zzU = disp_min[:, 0] * 100
                                                elif direc == 'Y' and ss == '正':
                                                    zzU = disp_max[:, 1] * 100
                                                elif direc == 'Y' and ss == '负':
                                                    zzU = disp_min[:, 1] * 100
                                                elif direc == 'Z' and ss == '正':
                                                    zzU = disp_max[:, 2] * 100
                                                elif direc == 'Z' and ss == '负':
                                                    zzU = disp_min[:, 2] * 100
                                                fig, xx, yy, zz, bds, hls, ccs = dyn_disp_contour(xxU, yyU, zzU, coordinate_nodes, bounds, self.nodes, holes, )
                                                self.figs.append(fig)
                                                self.figs_data.append([xx, yy, zz, bds, hls, ccs, coordinate_nodes])
                                                self.figs_type.append('contour')
                                                self.fig_title.append('%s剖面%s向最大%s位移包络等值线图(cm)' % (sec, direc, ss))
                                        else:
                                            self.write_log_to_Text('没有找到%s剖面的边界文件' % sec)
                    
                    # 应力等值线图的绘制
                    stre_sec = self.stre_section_ddl.current_value  # 获取剖面
                    stre_comp = self.stre_comp_ddl.current_value  # 获取分量
                            ...
                            if sec != '全选':
                                ...
                                for comp in stre_comp:
                                    if comp != '全选':
                                        if comp == '大主应力':
                                            comp_label = 'Smax'
                                            fname = os.path.join(odb_dir, 'dyn-stre-env-%s-max.csv' % sec)
                                        if comp == '小主应力':
                                            comp_label = 'Smin'
                                            fname = os.path.join(odb_dir, 'dyn-stre-env-%s-min.csv' % sec)
                                        if len(bounds) > 0:  # 绘图
                                            fig, xx, yy, zz, bds, hls, ccs = dyn_S_env_contour(fname, coordinate_nodes, bounds, self.nodes, comp_label, holes, concen=concen, concen_dist=0.01)
                                            self.figs.append(fig)
                                            self.figs_data.append([xx, yy, zz, bds, hls, ccs, coordinate_nodes])
                                            self.figs_type.append('contour')
                                            self.fig_title.append('%s剖面%s包络等值线图(MPa)' % (sec, comp))
                                        else:
                                            self.write_log_to_Text('没有找到%s剖面的边界文件' % sec)
                    # 横缝开度等值线图的绘制
                    joint_sec = self.joint_section_ddl.current_value  # 获取横缝
                    if joint_sec != '' :
                        if len(self.copen_data) == 0:  # 如果之前没有读取过横缝开度数据，则读取
                            dat_path = self.odb_path.get().replace('.odb', '.dat')
                            if os.path.exists(dat_path):
                                self.copen_data, self.copen_id = read_joint_dat_dyn(dat_path)  # 读取横缝开度数据
                                self.write_log_to_Text('横缝数据读取成功')
                        joint_sec = joint_sec.split(',')
                        for sec in joint_sec:
                            if sec != '全选':
                                bounds = os.path.join(self.geo_path.get(), 'bound-%s-X1.out' % sec)
                                coordinate = self.coord_all[sec]
                                joint = sec + '-X1'
                                fig, xx, yy, zz, bds, hls, ccs = dyn_joint_contour(joint, bounds, coordinate, self.copen_data, self.copen_id, self.nodes)
                                self.figs.append(fig)
                                self.figs_data.append([xx, yy, zz, bds, hls, ccs, coordinate])
                                self.figs_type.append('contour')
                                self.fig_title.append('%s横缝开度等值线图(mm)' % sec)
                    ...
                    if plane1 != '' and plane2 != '': 
                        stre_data1 = pd.read_csv(os.path.join(odb_dir, 'dyn-slid-%s.csv' % plane1), encoding='gbk')  # 读取滑面1的应力数据
                        stre_data1.rename(columns={'      结点编号': 'node', '         S-S11': 'S11', '         S-S22': 'S22', '         S-S33': 'S33', '         S-S12': 'S12', '         S-S13': 'S13', '         S-S23': 'S23'}, inplace=True)
                        stre_data2 = pd.read_csv(os.path.join(odb_dir, 'dyn-slid-%s.csv' % plane2), encoding='gbk')  # 读取滑面2的应力数据
                        stre_data2.rename(columns={'      结点编号': 'node', '         S-S11': 'S11', '         S-S22': 'S22', '         S-S33': 'S33', '         S-S12': 'S12', '         S-S13': 'S13', '         S-S23': 'S23'}, inplace=True)
                        ...
                        _, _, vec_n2 = get_vec_xyz(coord2, self.nodes)  # 花去滑面2的法向量
                        ...
                        # 求抗滑稳定安全系数
                        KK, fail_time = kanghua3D_2P_dyn(stre_data1, stre_data2, area_data1, area_data2, vec_n1, vec_n2, vec_t, f=[f1, f2], c=[c1, c2])
                        dt = 0.01
                        self.fail_time.set('%.2fs' %  (dt * fail_time)) 
                        fig, tt = dyn_slide(KK, dt)
                        self.figs.append(fig)
                        self.figs_data.append([tt, KK])
                        self.figs_type.append('history')
                        self.fig_title.append('%s抗滑稳定安全系数时程图' % plane1)
    ...
    def replot(self):
        ...
        contour_max = np.max(fig_data[2])
        contour_min = np.min(fig_data[2])
    ...
    def set_page(self):        
        ...
        func_label = tk.Label(self, text='三维时程法处理', font=('黑体', font_size1))
        ...
        self.acc_direc_ddl = Combopicker(self, values=['X', 'Y', 'Z', '全选'], entrywidth=14)
        ...
        acc_PGA_label.place(x=0.40 * w, y=acc_h * h, anchor='e')
        self.acc_PGA_entry = tk.Entry(self, textvariable='', width=7, relief='raised')
        self.acc_PGA_entry.place(x=0.40 * w, y=acc_h * h, anchor='w')
        acc_g_label = tk.Label(self, text='g', bg='white', font=('黑体', font_size3), fg='gray')
        acc_g_label.place(x=0.437 * w, y=acc_h * h, anchor='w')
        ...
        self.disp_section_ddl = Combopicker(self, values=[], entrywidth=14)
        ...
        disp_direc_label.place(x=0.28 * w, y=disp_h * h, anchor='e')
        self.disp_direc_ddl = Combopicker(self, values=['X', 'Y', 'Z', '全选'], entrywidth=14)
        self.disp_direc_ddl.place(x=0.28 * w, y=disp_h * h, anchor='w')
        ## 输入参考点
        disp_ref_label = tk.Label(self, text='参考点', bg='white', font=('黑体', font_size3), fg='gray')
        disp_ref_label.place(x=0.40 * w, y=disp_h * h, anchor='e')
        self.disp_ref_entry = tk.Entry(self, textvariable='', width=8, relief='raised')
        self.disp_ref_entry.place(x=0.40 * w, y=disp_h * h, anchor='w')
        ...
        ## 抗滑稳定安全校核不通过时长
        K_label = tk.Label(self, text='校核不通过时长', bg='white', font=('黑体', font_size3), fg='gray')
        K_label.place(x=0.24 * w, y=(slide_h + 0.08) * h, anchor='e')
        self.K_entry = tk.Entry(self, textvariable=self.fail_time, width=10, relief='raised', state='readonly')
        ...
        clear_entry_button.place(x=0.39 * w, y=0.62 * h, anchor='w')
        ...
        # self.fig_change_frame.place(x=0.564 * w, y=0.66 * h)
        ### 小数点位数
